...
def ren_main(local_file):
    # Download 'data.csv' from FTP server to the local directory
    download_csv_from_ftp(ftp_server_ip, ftp_username, ftp_password, 'data.csv', local_file)
...
# Submit Button
submit_button = tkinter.Button(frame, text="Submit", command=lambda: submit_data(ip_acl))
